...
void random_init() {
  if ((Random64[RandomNb-1] >> 32) != 0xF8D626AA) { 
              // upper half of the last element of the array
    ExitOn(EXIT_HARDQUIT,"random_init(): broken 64-bit types\n");
  }
}

uint64 random_64(int n) {
  if((n<0 )|| (n>=RandomNb)){
     ExitOn(EXIT_HARDQUIT,"Argument to random_64 out of range");
   }
   return Random64[n];
}

int piece_to_12(char p){
  char * pp;
  if((pp=strchr(PieceString,p))==NULL){
    ExitOn(EXIT_HARDQUIT,"Illegal argument for piece_to_12");
  }
  return(pp-PieceString);
}

int square_to_64(int f,int r){
  return f+8*r;
}
...
uint64 hash_piece_key(char p, int f, int r) {
  return random_64(RandomPiece+piece_to_12(p)*64+square_to_64(f,r));
}

int board_flags(IcsBoard *icsBoard){
   int flags;
   flags = 0;
   if (icsBoard->whiteCanCastleShort) flags |= 1 << 0;
   if (icsBoard->whiteCanCastleLong) flags |= 1 << 1;
   if (icsBoard->blackCanCastleShort) flags |= 1 << 2;
   if (icsBoard->blackCanCastleLong) flags |= 1 << 3;
   return flags;
}

uint64 hash_ep_key(int file) {
   return random_64(RandomEnPassant+file);
}

uint64 hash_turn_key(int c) {
  return (c=='W') ? random_64(RandomTurn) : 0;
}

uint64 hash_key(IcsBoard *icsBoard) {
  uint64 key=0;
  int r,f;
  char p;
  for(r=0;r<=7;r++){
    for(f=0;f<=7;f++){
      p=icsBoard->board[f][r];
      if(p!='-'){
	key^=hash_piece_key(p,f,r);
      }
    }
  }
  key ^= hash_castle_key(board_flags(icsBoard));
  f=icsBoard->epFile;
  if(f!=-1){
    if(icsBoard->onMove=='B'){ /* white to move */
      if((f>0 && ((icsBoard->board)[f-1][3]=='p'))|| 
         (f<7 && ((icsBoard->board)[f+1][3]=='p'))){
	key ^= hash_ep_key(f);
      }
    }else{ /* black to move */
      if((f>0 && ((icsBoard->board)[f-1][4]=='P'))|| 
         (f<7 && ((icsBoard->board)[f+1][4]=='P'))){
	key ^= hash_ep_key(f);
      }
    }
  }
  key ^= hash_turn_key(icsBoard->onMove);
  return key;
}
...
void book_open(const char file_name[]) {
   ...
   if (BookFile == NULL) ExitOn(EXIT_HARDQUIT,"Can't open book.");

   if (fseek(BookFile,0,SEEK_END) == -1) {
     ExitOn(EXIT_HARDQUIT,"Seek to en of book failed.");
   }

   BookSize = ftell(BookFile) / 16;
   if (BookSize == 0) ExitOn(EXIT_HARDQUIT,"Book is empty file.");
}
...
static uint64 read_integer(FILE * file, int size) {
   ...
   for (i = 0; i < size; i++) {
      ...
      if (b == EOF) {
         if (feof(file)) {
	   ExitOn(EXIT_HARDQUIT,"End of bookfile reached.");
         } else { // error
	   ExitOn(EXIT_HARDQUIT,"Unknown error reading bookfile");
         }
      }
      n = (n << 8) | b;
   }
...
static void read_entry(entry_t * entry, int n) {
   if (fseek(BookFile,n*16,SEEK_SET) == -1) {
     ExitOn(EXIT_HARDQUIT,"Error while reading book entry");
   }
...
double my_random_double() {
   double r;
   r = ((double) random()) / (((double)RAND_MAX) + 1.0);
   return r;
}
...
/* Below we ignore the possibility of hash collisions. */
void book_move(IcsBoard *icsBoard, book_move_t *bmove, Bool random) {
   int best_move;
   int best_score;
   int pos;
   entry_t entry[1];
   ...
   int score;
   uint64 key;
   int f,fr,ff,t,tf,tr,p;
   static char promote_pieces[5]={' ','n','b','r','q'};
   key=hash_key(icsBoard);
   best_move = MoveNone;
   best_score = 0;
   strcpy((bmove->move),"none");
   if(!BookFile) return;

   for (pos = find_pos(key); pos < BookSize; pos++) {
   ...
   (bmove->move)[3]=tr+'1';
   if(p){
     (bmove->move)[4]=promote_pieces[p];
     (bmove->move)[5]='\0';
   }else{
     (bmove->move)[4]='\0';
   }
   /* this strange castling convention has something to do with frc */
   if(!strcmp(bmove->move,"e1h1")){
     strcpy(bmove->move,"e1g1");
   }else  if(!strcmp(bmove->move,"e1a1")){
     strcpy(bmove->move,"e1c1");
   }else  if(!strcmp(bmove->move,"e8h8")){
     strcpy(bmove->move,"e8g8");
   }else  if(!strcmp(bmove->move,"e8a8")){
     strcpy(bmove->move,"e8c8");
   }
}
