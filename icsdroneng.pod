...
icsdroneng acts as an interface between an ICS (Internet Chess Server) and
...
When instructed to do so, icsdroneng will serve as a proxy for the ICS
it is connected to. This proxy works fairly well with xboard/winboard. 

In this case the setup is

     xboard--->icsdroneng--->ics

This is very similar to xboard with zippy except that now the engine remains
connected to the ICS even when xboard is disconnected. Furthermore
connecting xboard to icsdroneng is much faster than connecting it
directly to the ics as the login procedure is no longer necessary.
Finally xboard and icsdroneng do not have to be on the same machine
of course.
...
Specifies a ICS handle of a user which can send tell commands to icsdroneng.
...
=item B<-proxyFeedback> (default: "off")

Make icsdroneng give PV feedback directly to the proxy
...
address of the interface. The default insures
