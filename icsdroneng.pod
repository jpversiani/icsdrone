...
where xboard and icsdroneng do not have to be on the same machine.

Using the proxy with xboard is very similar to xboard with zippy
except that now the engine remains connected to the ICS even when
xboard is disconnected. Furthermore connecting xboard to icsdroneng is
much faster than connecting it directly to the ics as the login
