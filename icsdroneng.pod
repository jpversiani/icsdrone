...
=item B<-compactFeedback> (default: "off")

Give feedback on a single line as "<score>/<depth> <pv> ... [ <speed> ]"

=item B<-insertMoveNumbers> (default: "on")

Automatically insert any missing move numbers into principal variation.

=item B<-feedbackCommand> (default: "whisper" or "kibitz")

The command to use to send feedback to observers. Use "kibitz" if you want your
opponent to also see the feedback.

If not set, "whisper" will be used when playing humans and "kibitz"
when playing computers.
...
Send "rematch" at end of the game. If the last opponent was a computer,
repeat after every 10 minutes of idle time.
